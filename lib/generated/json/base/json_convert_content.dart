...
import 'package:blog_project/generated/json/weather_bean_entity_helper.dart';
...
class JsonConvert<T> {
  ...
  static _getFromJson<T>(Type type, data, json) {
    switch (type) {
			...
			case WeatherBeanEntity:
				return weatherBeanEntityFromJson(data as WeatherBeanEntity, json) as T;
			case WeatherBeanWeatherinfo:
				return weatherBeanWeatherinfoFromJson(data as WeatherBeanWeatherinfo, json) as T;
			case TokenEntity:
  ...
  static _getToJson<T>(Type type, data) {
		switch (type) {
			...
			case WeatherBeanEntity:
				return weatherBeanEntityToJson(data as WeatherBeanEntity);
			case WeatherBeanWeatherinfo:
				return weatherBeanWeatherinfoToJson(data as WeatherBeanWeatherinfo);
			case TokenEntity:
  ...
  //Go back to a single instance by type
	static _fromJsonSingle<M>( json) {
		...
		if(type == (ArtileTitleItemEntity).toString()){
		...
		}	else if(type == (WeatherBeanEntity).toString()){
			return WeatherBeanEntity().fromJson(json);
		}	else if(type == (WeatherBeanWeatherinfo).toString()){
			return WeatherBeanWeatherinfo().fromJson(json);
  ...
  //list is returned by type
	static M _getListChildType<M>(List data) {
		...
		}	else if(<WeatherBeanEntity>[] is M){
			return data.map<WeatherBeanEntity>((e) => WeatherBeanEntity().fromJson(e)).toList() as M;
		}	else if(<WeatherBeanWeatherinfo>[] is M){
			return data.map<WeatherBeanWeatherinfo>((e) => WeatherBeanWeatherinfo().fromJson(e)).toList() as M;
